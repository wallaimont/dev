...
import type { PedidoVenda, Column } from '@/types';

const columns: Column<PedidoVenda>[] = [
...
function ExtraActions({ item, reload }: { item: PedidoVenda; reload: () => void }) {
  const action = async (acao: string) => {
    try {
      await api.patch(`/vendas/pedidos/${item.id}/${acao}`);
